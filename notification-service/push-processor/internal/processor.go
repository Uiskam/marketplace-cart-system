...
const TaskPush = "notification:push"
...
func (p *Processor) ProcessTask(ctx context.Context, task *asynq.Task) error {
	...
	if rand.Float64() < 0.5 {
		...
		return fmt.Errorf("push processor failed to process notification")
	}
	...
	fmt.Printf("[%s] Successfully processed push notification to %s: %s\n",
